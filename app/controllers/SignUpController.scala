...
class SignUpController @Inject() (
  ...
  def submit = UnsecuredAction.async { implicit request: Request[AnyContent] =>
    SignUpForm.form.bindFromRequest.fold(
      ...
      data => {
        val result = Redirect(controllers.routes.SignUpController.view()).flashing("info" -> Messages("sign.up.email.sent", data.email))
...
//              val url = controllers.routes.ActivateAccountController.activate(authToken.id).absoluteURL()
//              mailerClient.send(Email(
